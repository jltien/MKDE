...
gridFloat * mkde2dGridv02interact(const vector<double> &T, const vector<double> &X0, vector<double> &Y0,
    ...
    totalT = 0.0;
    /*
     * FROM HERE UNTIL THE LINE "while (!exitLoop)" WE HAVE:
     * 1. A LOOP TO GO THROUGH EACH OBSERVED MOVE STEP
     * 2. A CONDITIONAL STATEMENT TO CHECK IF THE MOVE STEP IS TO BE USED
     * 3. A WHILE LOOP FOR INTEGRATION TIME STEPS 
     * 
     * THESE CAN BE REPLACED WITH A SINGLE LOOP TO GO THROUGH EACH
     * RECORD IN THE OUTPUT OF THE INTERACTION FUNCTION AND
     * A CONDITIONAL TO CHECK IF THE MOVE IS VALID...
     */
    for (int j = 0; j < (nObs - 1); j++) {
        cout << "\tProcessing move step " << (j + 1) << " of " << (nObs - 1) << endl;
    // report percent complete after each observation
        ...
        if (isValid[j] == 1) {
            ...
            while (!exitLoop) { // iterate over integration time steps
                ...
                /*
                 * START: MOVE VARIANCE AND INTERPOLATED LOCATIONS
                 * INDIVIDUAL ALPHA, MOVE VARIANCE, AND EXPECTED (X,Y) 
                 * COORDINATES SHOULD BE ALREADY CALCULATED FOR THIS
                ...
                /*
                 * END: MOVE VARIANCE AND INTERPOLATED LOCATIONS
                 */
                ...
                /*
                 * SINCE WE ALREADY HAVE A GRID OF INTEGRATION TIME STEPS
                 * TO SCAN THROUGH, WE PROBABLY DON'T NEED THESE LINES
                 * TO DETERMINE IF WE SHOULD EXIT THE WHILE LOOP...
                 */
                // update the eval time (t) and stopping conditions
                if (finalLoop) {
                    exitLoop = true;
            ...
            } // **** END WHILE LOOP FOR TIME STEPS ****
        } // **** END OF BLOCK WRAPPED IN CONDITIONAL TO CHECK FOR VALID MOVE ****
    } // **** END FOR LOOP FOR OBSERVED MOVE STEPS ****
    // divide by totalT
